...
public static class CanvasExt {
	...
	public static Vector2 WorldToCanvasPoint(this Canvas canvas, Camera camera, Vector3 worldPoint) {
		...
		// convert the 3D position where the bubble should appear to 2D coordinates for the canvas
		Vector3 worldPos = worldPoint;
		Vector2 viewportPos = camera.WorldToViewportPoint(worldPos);
		return canvas.ViewportToCanvasPoint(viewportPos);
	}

	public static Vector2 ScreenToCanvasPoint(this Canvas canvas, Vector2 screenPosition)
	{
		var viewportPosition = new Vector3(screenPosition.x / Screen.width,
										   screenPosition.y / Screen.height,
										   0);
		return canvas.ViewportToCanvasPoint(viewportPosition);
	}

	public static Vector2 ViewportToCanvasPoint(this Canvas canvas, Vector2 viewportPosition)
	{
	...
	public static Vector2 CanvasToViewportPoint(this Canvas canvas, Vector2 canvasPosition)
	{
		var canvasRect = canvas.GetComponent<RectTransform>();
		var scale = canvasRect.sizeDelta;
	...
	{
		var viewportPosition = canvas.CanvasToViewportPoint(canvasPoint);
		return new Vector2(viewportPosition.x * Screen.width, viewportPosition.y * Screen.height);
	}

	public static Vector2 EventToCanvasPoint(this Canvas canvas, PointerEventData evt)
	{
		return evt.position / canvas.scaleFactor;
	}
